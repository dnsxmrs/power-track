...
import type { SubscriptionItem } from '../page';
import type { SubscriptionDecision } from './reviewsubscription';
...
interface ConfirmationModalProps {
	...
	decision: SubscriptionDecision | null;
	isSubmitting?: boolean;
}

const OUTCOME_META: Record<SubscriptionDecision['status'], { label: string; icon: typeof BadgeCheck; className: string }> = {
	active: {
		label: 'Keep active',
		icon: BadgeCheck,
...
export function ConfirmationModal({ isOpen, onClose, onConfirm, subscription, decision, isSubmitting = false }: ConfirmationModalProps) {
	if (!isOpen || !subscription || !decision) {
		return null;
	}
						...
						<div className={`rounded-2xl border p-4 ${meta.className}`}>
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<p className="text-sm opacity-90 break-words">{subscription.planName}</p>
								</div>
							</div>
						</div>
