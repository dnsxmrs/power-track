...
export function AddPlanModal({ isOpen, onClose, onSubmit }: Props) {
  const [name, setName] = useState('');
  const [slug, setSlug] = useState('');
  const [monthlyPrice, setMonthlyPrice] = useState('0');
  const [deviceCap, setDeviceCap] = useState('1');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <h3 className="text-lg font-semibold text-white mb-4">Add Subscription Plan</h3>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button type="submit" className="px-4 py-2 rounded-lg bg-cyan-500/20 text-cyan-400">Create</button>
        </div>
      </motion.form>
    </div>
  );
